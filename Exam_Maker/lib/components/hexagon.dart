...
class HexagonPatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final numberOfColumns =
        ((size.width + spacing) / (hexagonSize + spacing)).ceil();
    final numberOfRows =
        ((size.height + spacing) / (hexagonSize * 0.866 + spacing)).ceil();

    for (var i = 0; i < numberOfRows; i++) {
      for (var j = 0; j < numberOfColumns; j++) {
        final path = Path();
        final centerX = j * (hexagonSize + spacing) +
            (i.isOdd ? (hexagonSize + spacing) / 2 : 0);
